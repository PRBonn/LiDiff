...
class DiffusionPoints(LightningModule):
    ...
    def p_sample_loop(self, x_init, x_t, x_cond, x_uncond, x_mean, x_std, generate_viz=False):
        self.scheduler_to_cuda()
        ...
        for t in tqdm(range(len(self.dpm_scheduler.timesteps))):
            t = torch.ones(x_init.shape[0]).cuda().long() * self.dpm_scheduler.timesteps[t].cuda()
            ...
            if generate_viz:
                viz = self.visualize_step_t(x_t, x_init.cpu().detach().numpy(), viz_pcd, x_mean, x_std)
